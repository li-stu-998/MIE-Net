...
# options
parser = argparse.ArgumentParser(description="MIE testing on the full validation set")
...
def accuracy(output, target, topk=(1,)):
    ...
    pred = pred.t()
    correct = pred.eq(target.view(1, -1).exMIEd_as(pred))
    res = []
    for k in topk:
...
for this_weights, this_test_segments, test_file in zip(weights_list, test_segments_list, test_file_list):
    ...
    data_loader = torch.utils.data.DataLoader(
            MIEDataSet(root_path, test_file if test_file is not None else val_list, num_segments=this_test_segments,
